...
export function AppShell({ header, sidebar, main, aside, overlays }: AppShellProps) {
  return (
    <div className="app-bg flex min-h-screen flex-col">
      {header}
      <div
        className={`app-container grid flex-1 gap-4 lg:gap-6 ${
          aside
      ...
      >
        {/* Sidebar shows inline only on lg+. On mobile/tablet the sidebar is
            available via a drawer triggered from the header, so we hide the
            inline copy to avoid pushing the board far down the page. */}
        <aside className="hidden min-w-0 lg:block lg:sticky lg:top-[92px] lg:self-start">
          {sidebar}
        </aside>
