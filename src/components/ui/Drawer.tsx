...
export function Drawer({
  ...
  const shape =
    side === 'bottom'
      ? 'max-h-[82svh] w-full rounded-t-2xl'
      : 'h-full w-full max-w-[min(440px,92vw)]'
  ...
  return (
    <div className={`fixed inset-0 z-40 flex ${align}`}>
      ...
      <aside className={`relative flex flex-col bg-white shadow-pop ${shape} ${className}`}>
        <header className="flex items-center justify-between border-b border-ink-200 px-4 py-3 sm:px-6 sm:py-4">
